...
- 사용자 직감 가능 시간 투표 화면 (`/availability`): 타임별 불가능/가능/⭐ 선호 3단계 선택,
  기본값 전부 불가능, "이번 주에는 모든 시간에 직감을 설 수 없습니다" 일괄 체크
...
- `src/lib/assignment/fairness-engine.ts`: DB/Next.js에 의존하지 않는 주간 전체 최적화 알고리즘.
  최소비용 최대유량으로 결원 최소화 → 주간 횟수 균등 → 가능 타임 수 → 선호 타임 → 헬스장 선호 →
  Seed Random 순서를 엄격하게 적용한다. 재실행 시 이미 채워진 자리와 해당 배정 횟수도 반영한다.
...
- 배정 결과 검증(각 슬롯 힘레븐1/2 각 1명, 동일 날짜 중복 없음, 현재 제약에서 공평성 최적)과 미배정
  슬롯 처리(§74-76): 후보가 부족하면 임의로 중복 배정하지 않고 해당 슬롯을 `UNDERSTAFFED`로 표시.
