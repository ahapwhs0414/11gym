# 헬스장 직감 관리 시스템 (Phase 7)

힘레븐1 / 힘레븐2 직감 배정 및 관리 시스템입니다. 현재 이 저장소는 **Phase 7**까지 구현되어 있습니다.
...
아직 없는 것(다음 Phase 예정): 관리자 통계/공지/의견/감사 로그, PWA 등.
...
- 관리자 월간 직감/출석 기록, 공평성 통계, 의견 관리, 공지 관리, 감사 로그 조회, 시스템 설정
  (지각 유예시간 등 지금은 코드에 하드코딩된 값들을 설정 화면으로 이동)
