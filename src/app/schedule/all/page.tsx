...
function AttendanceInfo({ dutyLog }: { dutyLog: DutyLog | undefined }) {
...
/** §36: 관리자뿐 아니라 일반 사용자도 전체 사용자의 직감 일정을 조회할 수 있다(읽기 전용). */
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-3xl px-4 py-8">
        ...
        <div className="mb-4 flex items-center justify-between">
          <Link
            href={`/schedule/all?week=${prevWeek}`}
          ...
          <Link
            href={`/schedule/all?week=${nextWeek}`}
            className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm text-slate-600"
          >
            다음 주 →
          </Link>
          ...
          <table className="w-full min-w-[480px] text-left text-sm">
            ...
            <tbody>
              {expectedRows.map((row) => {
                ...
                const gym1 = slot?.assignments.find(
                  (a: DutyAssignment & { user: User; gym: Gym }) => a.gym.name === "힘레븐1"
                );
                const gym2 = slot?.assignments.find(
                  (a: DutyAssignment & { user: User; gym: Gym }) => a.gym.name === "힘레븐2"
                );
                ...
                return (
                  <tr key={key} className="border-b border-slate-100">
                    ...
                    <td className="px-4 py-2">
                      {gym1 ? (
                        <>
                          <p className="text-slate-900">{gym1.user.name}</p>
                          <AttendanceInfo dutyLog={dutyLogByAssignmentId.get(gym1.id)} />
                        </>
                      ) : (
                    ...
                    <td className="px-4 py-2">
                      {gym2 ? (
                        <>
                          <p className="text-slate-900">{gym2.user.name}</p>
                          <AttendanceInfo dutyLog={dutyLogByAssignmentId.get(gym2.id)} />
                        </>
                      ) : (
                        <span className="text-red-500">미배정</span>
                      )}
